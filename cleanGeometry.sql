-- Executando função cleangeometry(geometry)
DROP TABLE nazaria_clean;
CREATE TABLE nazaria_clean AS
(WITH r AS
	(SELECT
		class_name,
		cleangeometry(geom) AS geom
	FROM
		nazaria_orig
	ORDER BY
		class_name ASC)
...
ALTER TABLE nazaria_clean ADD PRIMARY KEY (gid);

ALTER TABLE nazaria_clean
ALTER COLUMN geom TYPE geometry(Polygon, 32723)
USING ST_Transform(geom, 32723);

CREATE INDEX nazclean ON nazaria_clean USING GIST(geom);
